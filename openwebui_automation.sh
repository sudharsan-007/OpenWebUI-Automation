...
display_help() {
  ...
  echo -e "  -d, --dir      Specify installation directory (default: $INSTALL_DIR)"
  echo -e "  -p, --port     Specify host port (default: $HOST_PORT)"
...
check_prerequisites() {
  ...
  for tool in git curl docker dialog jq; do
  ...
  if [ ${#missing_tools[@]} -gt 0 ]; then
    ...
    echo
    echo -e "${YELLOW}Would you like the script to attempt to install these tools? [y/N]${NC}"
  ...
  if ! NO_DIALOG && ! dialog --version > /dev/null 2>&1; then
...
dialog_prompt() {
  ...
  dialog --backtitle "OpenWebUI Deployment" --title "$title" --inputbox "$message" 8 60 2> "$temp_file"
  local retval=$?
...
terminal_checklist() {
  ...
  echo -e "${CYAN}$message${NC}"
  echo -e "${YELLOW}Use space to select/deselect options, then press Enter when done.${NC}"
  ...
  echo
  echo -e "${YELLOW}Enter the numbers of options to toggle (comma or space separated):${NC}"
  read -r selections
...
prompt_config() {
  ...
  if [ "$NO_DIALOG" = true ]; then
    ...
    host_port=$(dialog_prompt "Host Port" "Enter the host port for OpenWebUI (default: $HOST_PORT)")
  ...
  api_items=(
    "ollama" "Ollama API (local AI models)" "on"
...
EOF
  ...
  # API configurations
  if [ "$(echo "$api_config" | jq -r '.ollama // false')" = "true" ]; then
    cat >> "$INSTALL_DIR/$DOCKER_COMPOSE_FILE" <<EOF
...
EOF
  ...
  if [ "$(echo "$api_config" | jq -r '.claude // false')" = "true" ]; then
    local claude_key=$(echo "$api_config" | jq -r '.claude_key // ""')
    # The Claude API will be set up through the OpenAI-compatible connection in the WebUI
  fi
  
  if [ "$(echo "$api_config" | jq -r '.openrouter // false')" = "true" ]; then
    local openrouter_key=$(echo "$api_config" | jq -r '.openrouter_key // ""')
    # The OpenRouter API will be set up through the OpenAI-compatible connection in the WebUI
  fi
...
deploy_openwebui() {
  ...
  if [ "$NO_DIALOG" = true ]; then
    if ! terminal_yesno "Start the deployment now?" "y"; then
      log "INFO" "Deployment prepared but not started"
      return 0
    fi
  else
    if ! dialog_yesno "Start Deployment" "Start the deployment now?" "y"; then
      log "INFO" "Deployment prepared but not started"
      return 0
    fi
  fi
  ...
  echo -e "- View logs: ${BOLD}docker logs open-webui${NC}"
  ...
  echo -e "- Restart service: ${BOLD}docker restart open-webui${NC}"
  echo
...
# Main function
main() {
  # Process command line arguments (already done at the beginning)
